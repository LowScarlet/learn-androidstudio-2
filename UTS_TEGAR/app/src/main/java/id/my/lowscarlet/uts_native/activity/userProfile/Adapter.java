package id.my.lowscarlet.uts_native.activity.userProfile;
...
public class Adapter extends RecyclerView.Adapter<Adapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        // TODO Edit This Scarlet
        TextView fullName = Utils.createTextView(context);
        viewHolder.itemCountainer.addView(fullName);
        TextView bio = Utils.createTextView(context);
        viewHolder.itemCountainer.addView(bio);
        TextView userId = Utils.createTextView(context);
        viewHolder.itemCountainer.addView(userId);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        // TODO Edit This Scarlet
        ((TextView) holder.itemCountainer.getChildAt(0)).setText(String.format("FullName: %s", data.fullName));
        ((TextView) holder.itemCountainer.getChildAt(1)).setText(String.format("Bio: %s", data.bio));
        ((TextView) holder.itemCountainer.getChildAt(2)).setText(String.format("UserId: %s", data.userId));
    ...
    private void showUpdateModal(Model model) {
        ...
        // TODO Edit This Scarlet
        EditText fullName = Utils.createEditText(context, "fullName");
        fullName.setText(model.fullName);
        updateContainer.addView(fullName);
        EditText bio = Utils.createEditText(context, "bio");
        bio.setText(model.bio);
        updateContainer.addView(bio);
        EditText userId = Utils.createEditText(context, "userId");
        userId.setText(model.userId);
        updateContainer.addView(userId);
        ...
        updateButton.setOnClickListener(v -> {
            ...
            if (!Objects.equals(model.bio, bio.getText().toString())) {
                updatedModel.put("bio", bio.getText().toString());
            }

            if (!Objects.equals(model.userId, userId.getText().toString())) {
                updatedModel.put("userId", userId.getText().toString());
            }
