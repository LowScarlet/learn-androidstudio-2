...
public class Activity extends AppCompatActivity {
    private Retrofit retrofit = id.my.lowscarlet.uts_native.Config.retrofit;
    ...
    private void showCreateModal() {
        ...
        EditText pekerjaan = Utils.createEditText(this, "Pekerjaan");
        inputContainer.addView(pekerjaan);
    ...
    public void updateData(Model model, AlertDialog dialog) {
        String dataId = model.id;
        Call<GetResponse<Model>> call = handler.updateData(dataId, model);

        call.enqueue(new Callback<GetResponse<Model>>() {
            @Override
            public void onResponse(Call<GetResponse<Model>> call, Response<GetResponse<Model>> response) {
                if (response.isSuccessful()) {
                    GetResponse<Model> updateResponse = response.body();
                    Model updatedData = updateResponse.getData();
    ...
    public void deleteData(Model model, AlertDialog dialog) {
        String dataId = model.id;
        Call<GetResponse<Model>> call = handler.deleteData(dataId);
